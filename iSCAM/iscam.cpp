...
model_data::model_data(int argc,char * argv[]) : ad_comm(argc,argv)
{
 ...
 age.fill_seqadd(sage,1);
  ...
  fsh_flag.allocate(1,ngear,"fsh_flag");
  ft_phz.allocate(1,ngear);
		int k;
		ft_phz=1;
		for(k=1;k<=ngear;k++)
		{
  ...
  survey_data.allocate(1,nit,1,nit_nobs,1,4,"survey_data");
 ...
 model_data(argc,argv) , function_minimizer(sz)
{
 ...
 for(int i=1;i<=npar;i++) theta(i)=theta_ival(i);
  sel_par.allocate(1,ngear,1,jsel_npar,1,isel_npar,-5.,5.,sel_phz,"sel_par");
 ...
 if(cntrl(5)) ii = syr;  //if initializing with ro
  log_rec_devs.allocate(ii,nyr,-15.,15.,2,"log_rec_devs");
  f.allocate("f");
  ro.allocate("ro");
  ...
  nlvec.allocate(1,6,1,ilvec,"nlvec");
    ...
    nlvec.initialize();
  #endif
  jlog_sel.allocate(1,ngear,sage,nage,"jlog_sel");
  #ifndef NO_AD_INITIALIZE
    jlog_sel.initialize();
  #endif
...
{
  dvector temp1("{100,100,500,5000,5000}");
  maximum_function_evaluations.allocate(temp1.indexmin(),temp1.indexmax());
  maximum_function_evaluations=temp1;
  dvector temp("{0.01,0.01,1.e-4,1.e-4}");
...
void model_parameters::calc_numbers_at_age(void)
{
	int i,j;
	N.initialize();
...
void model_parameters::calc_age_proportions(void)
{
...
void model_parameters::calc_fishery_observations(void)
{
	int i,k;
	for(i=syr;i<=nyr;i++)
	{
...
void model_parameters::calc_survey_observations(void)
{
