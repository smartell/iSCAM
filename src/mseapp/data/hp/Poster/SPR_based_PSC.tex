...
\begin{columns}
	...
	\block{Key Points}{
		\begin{itemize}
			...
			\item Joint management requires an allocation agreement.
			\item Allocations can be based on yield per recruit (\textbf{YPR}), or mortality per recruit (\textbf{MPR}).
	...
	\column{0.30}
	\block{Joint Management}{
		\begin{itemize}
			\item A harvest policy for joint managment requires an allocation agreement.
		...
		\end{itemize}
		\begin{tikzfigure}[Average removals between 1990 and 2014, proportion yield removed, and the precent of the .]
			\resizebox{\linewidth}{!}{
				\begin{tabular}{ l|r r r}
					\hline
					Sector & Removals (Mlb)  & YPR proportion & MPR proportion \\
					\hline
					Commercial & 59730  & 72.6\%& 35.7\%\\
					Bycatch    & 13298  & 16.5\%& 54.2\%\\
					Sport      &  8285  & 10.4\%&  9.1\%\\
					Personal   &  1051  &  1.3\%&  0.8\%\\
					\hline
				\end{tabular}
