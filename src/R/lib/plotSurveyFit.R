...
.plotSurveyFit <- function(M)
{
	...
	for( i in 1:n )
	{
		...
		Ub <- log(M[[i]]$d3_survey_data[,2])+1.96*(1/M[[i]]$d3_survey_data[,7])*M[[i]]$d3_survey_data[,2]
		Lb <- log(M[[i]]$d3_survey_data[,2])-1.96*(1/M[[i]]$d3_survey_data[,7])*M[[i]]$d3_survey_data[,2]
	...
	p <- p + geom_pointrange(aes(Year,Index,max=exp(high_bnd), ymin=exp(low_bnd),color=factor(Gear)))
