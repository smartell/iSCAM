...
\subsubsection{Deterministic age-structured model} % (fold)
...
% subsubsection deterministic_age_structured_model (end)
...
\subsubsection{Stochastic age-structured model} % (fold)
\label{ssub:stochastic_age_structured_model}
    ...
    \begin{align}
        ...
        % R_i &= \begin{cases}
            % \dfrac{s_o B_{i-1}} {1+ \beta B_{i-1}} e^\omega_i, &\mbox{Beverton-Holt} \\[1ex]
            % s_o B_{i-1} \exp(-\beta B_{i-1} + \omega_i), &\mbox{Ricker}
        % \end{cases}                                                     \label{T3.5} \\
        %%
        %%
...
% The estimated parameter vector in \iscam\ is defined in \eqref{T3.1}, where $R_0, \kappa$ and $M$ are the leading unknown population parameters that define the overall population scale in the form of unfished recruitment and productivity in the form of recruitment compensation and natural mortality.  





% The total variance $\vartheta^2$ and the proportion of the total variance that is associated with observation errors $\rho$ are also estimated, then the variance is partitioned into observation errors ($\sigma^2$) and process errors ($\tau^2$) using \eqref{T4.2}.
...
% subsubsection stochastic_age_structured_model (end)


\subsubsection{Fisheries selectivity} % (fold)
\label{ssub:fisheries_selectivity}

% subsubsection fisheries_selectivity (end)
