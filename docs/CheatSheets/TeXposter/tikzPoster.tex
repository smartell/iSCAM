...
\begin{columns}
	...
	\block{Allocation constraints}{
		In cases where there is a fixed allocation among fleets, solving the system of equations involves finding a single average fishing mortality rate ($f^*$), and using a multiplier to derive $f_k = f^* \lambda_k$. The multiplier has an analytical solution that is based on the relative yield per recruit.
