\documentclass[17pt, a0paper, landscape,colspace=1cm]{tikzposter}
...
\newcommand{\dye}    { \dfrac{{\partial y_k}}{{\partial f_{k'}}} }%
...
\newcommand{\ddye}   { \dfrac{{\partial^2 y_k}}{{\partial f_{k'}}^2} }%
...
\begin{columns}
	...
	\block{Catch Equation \& Finding $\bm{f^*}$}{
		...
		\begin{align}
			...
			%
		\end{align}
		To find the vector of fishing mortality rates that simultaneously maximizes the yield for each fleet we use Newton's method to find the solution to $\dfrac{\partial \vec{y}}{\partial \vec{f}} = 0$. 

		 The first and second derivatives of \eqref{eq.1} for fleet $k$ are given by:
		\begin{align}
			%
			\mbox \nonumber \\
			\dye &=
		...
		\begin{align}
			\vec{f}_{i+1} = \vec{f}_{i} - \dfrac{\partial y}{\partial f}
			 \left( \dfrac{{\partial^2 y}}{{\partial f}^2} \right)^{-1} \label{eq.4}
		\end{align}
	}% end block.
...
\begin{columns}
	...
	\block{Allocation constraints}{
		In cases where there is an explicit  agreement where each fleek $k$ is allocation a portion of the total catch, solving the system of equations involves finding a single average fishing mortality rate ($f^*$), and using a multiplier to derive $f_k = f^* \lambda_k$. The multiplier has an analytical solution that is based on the relative yield per recruit.

		\begin{align}
			f_k &= f^* \lambda_k \\
			\lambda_k &= \dfrac{\Phi a_k}{\phi_k \left( \sum_k \dfrac{a_k \Phi}{\phi_k}\right)}\\ 
			\mbox{where} \nonumber\\
			\Phi &= \sum_k \phi_k
		\end{align}
	}
	...
	\block{Post-its}{
		% \centering
		\includegraphics[width=0.40\linewidth,keepaspectratio=true,trim=-5cm 0 0 15cm]{./whiteBoard.png}
	}
	\useblockstyle{TornOut}
	%
	% MSY Figure
	...
	\note[
		...
		\begin{tikzfigure}[\Large Equilibrium yield versus fishing mortality for 2 fishing gears. Which fishery (A or B) catches smaller fish?]
	 	\label{fig.MSY}
	 	\includegraphics[width=15cm]{figMSY.png}
		\end{tikzfigure}
	}
